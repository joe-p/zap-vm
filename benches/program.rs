...
fn create_large_bytecode() -> Vec<u8> {
    ...
    for i in 0..500 {
        ...
        if i % 5 == 0 {
            ...
            bytecode.push(opcodes::REG_STORE);
            bytecode.push(opcodes::REG_LOAD);
        } else {
            bytecode.push(opcodes::INIT_VEC_WITH_INITIAL_CAPACITY);
...
fn bench_parse_instructions(c: &mut Criterion) {
    ...
    group.bench_function("small_program", |b| {
        b.iter(|| {
            Instruction::from_bytes(black_box(&small_bytecode)).unwrap()
        });
    });
    
    group.bench_function("medium_program", |b| {
        b.iter(|| {
            Instruction::from_bytes(black_box(&medium_bytecode)).unwrap()
        });
    });
    
    group.bench_function("large_program", |b| {
        b.iter(|| {
            Instruction::from_bytes(black_box(&large_bytecode)).unwrap()
        });
    });
